...
class ConditionalRandomField_2(torch.nn.Module):
    """
    ...
    def _input_likelihood(self, logits: torch.Tensor, mask: torch.BoolTensor) -> torch.Tensor:
        ...
        for i in range(1, sequence_length):
            ...
            alpha = util.logsumexp_loss(inner, 1) * mask[i].view(batch_size, 1) + alpha * (
                ~mask[i]
            ).view(batch_size, 1)
        ...
        return util.logsumexp_loss(stops)
    ...
    def forward(
        self, inputs: torch.Tensor, tags: torch.Tensor, mask: torch.BoolTensor = None, alpha:  float = 1, gamma: float = 2
    ) -> torch.Tensor:
        """
        ...
        log_init = log_numerator - log_denominator
        log_init2 = torch.sum(log_init)
        #return torch.sum(log_numerator - log_denominator)
        return torch.sum(alpha*(torch.pow(1-log_init.exp(), gamma))*log_init)
        #return alpha * (torch.pow(1 - log_init2.exp(), gamma)) * log_init2
